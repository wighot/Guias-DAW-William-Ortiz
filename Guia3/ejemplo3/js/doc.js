...
function init(){
    ...
    aleatorios = [];
    //Declaración y asignación de los límites entre los cuales estarán comprendidos los números
    aleatorios
    min = parseInt(prompt("Ingrese el límite inferior",""));
    max = parseInt(prompt("Ingrese el límite superior",""));
    cantidad = parseInt(prompt("Indique la cantidad denúmeros aleatorios a generar", ""));
    ...
    if(disparador.addEventListener){
        ...
        function(evt){
            ...
            listado.innerHTML = "Los números aleatoriosson: " + aleatorios.toString();
        }, false);
...
function aleatorio(minimo, maximo, cantidad){
    ...
    //Generando un número aleatorio teniendo el cuidado que esté comprendido entre los valores mínimo y
    máximo
    var numero = Math.floor(Math.random() * (maximo - minimo + 1)) + minimo;
    //Verificar que no se haya llegado a la cantidad de números aleatorios máximo establecidos
    if(aleatorios.length < cantidad){
        //Agregar el número aleatorio al arreglo o matriz utilizando el método push()
